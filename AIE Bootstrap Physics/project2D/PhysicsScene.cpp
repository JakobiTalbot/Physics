...
void PhysicsScene::Update(float fDeltaTime)
{
	...
	static float fDebugAccumalatedTime = 0.f;
	fDebugAccumalatedTime += fDeltaTime;
...
bool PhysicsScene::Plane2AABB(PhysicsObject* pObject1, PhysicsObject* pObject2)
{
	...
	float r = pAABB->GetExtent().x * fabs(pPlane->GetNormal().x) + pAABB->GetExtent().y * fabs(pPlane->GetNormal().y);

	// find distance between plane and AABB
	float fDistance = glm::dot(pPlane->GetNormal(), pAABB->GetPosition()) - pPlane->GetDistance();
...
bool PhysicsScene::Sphere2AABB(PhysicsObject* pObject1, PhysicsObject* pObject2)
{
	...
	if (glm::distance(v2ClosestPoint, pSphere->GetPosition()) < pSphere->GetRadius())
	{
		...
		if (glm::length(v2CollisionNormal) > 0.f)
		{
			v2CollisionNormal = glm::normalize(v2CollisionNormal);
		}
		else
		{
			v2CollisionNormal = glm::normalize(pSphere->GetPosition() - pAABB->GetPosition());
		}
